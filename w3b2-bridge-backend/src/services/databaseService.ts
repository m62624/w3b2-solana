...
export class DatabaseService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await fs.mkdir(path.join(this.dataDir, 'funding_requests'), { recursive: true });
  ...
  async createUser(userAccount: UserAccount): Promise<void> {
    const filePath = path.join(this.dataDir, 'users', `${userAccount.public_key.toBase58()}.json`);
    const data = {
      ...userAccount,
      created_at: Date.now(),
      updated_at: Date.now(),
  ...
  async getUser(publicKey: PublicKey): Promise<UserAccount | null> {
    try {
      const filePath = path.join(this.dataDir, 'users', `${publicKey.toBase58()}.json`);
  ...
  async updateUser(publicKey: PublicKey, updates: Partial<UserAccount>): Promise<void> {
  ...
  async createAdmin(adminAccount: AdminAccount): Promise<void> {
    const filePath = path.join(this.dataDir, 'admins', `${adminAccount.public_key.toBase58()}.json`);
    const data = {
      ...adminAccount,
      created_at: Date.now(),
      updated_at: Date.now(),
    ...
    await fs.writeFile(filePath, JSON.stringify(data, null, 2));
    console.log(`👑 Администратор создан: ${adminAccount.public_key.toBase58()}`);
  }

  async getAdmin(publicKey: PublicKey): Promise<AdminAccount | null> {
    try {
      const filePath = path.join(this.dataDir, 'admins', `${publicKey.toBase58()}.json`);
      const data = await fs.readFile(filePath, 'utf8');
  ...
  async createFundingRequest(fundingRequest: FundingRequest): Promise<string> {
    ...
    const filePath = path.join(this.dataDir, 'funding_requests', `${requestId}.json`);
  ...
  async updateRecord(recordId: string, updates: any, owner: PublicKey): Promise<void> {
  ...
  async handleCrudOperation(operation: CrudOperation, owner: PublicKey): Promise<any> {
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      const [usersFiles, adminsFiles, fundingRequestsFiles, recordsFiles] = await Promise.all([
      ...
      return {
        ...
        fundingRequests: fundingRequestsFiles.filter(f => f.endsWith('.json')).length,
